...
package dev.masterfile_integration_bdd_0_33;
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_8", "", Thread.currentThread().getId() + "", "INFO",
							"", "Job en succès", "", "");
					talendLogs_LOGS.addMessage("tWarn", "tWarn_8", 3, "Job en succès", 3);
					talendLogs_LOGSProcess(globalMap);
					globalMap.put("tWarn_8_WARN_MESSAGES", "Job en succès");
	...
	public void tRunJob_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				obj_tRunJob_2 = context.emailErrorRecipient;
				if (obj_tRunJob_2 != null) {
					paraList_tRunJob_2
							.add("--context_param recipientEmail=" + RuntimeUtils.tRunJobConvertContext(obj_tRunJob_2));
				} else {
					paraList_tRunJob_2.add("--context_param recipientEmail="
							+ NULL_VALUE_EXPRESSION_IN_COMMAND_STRING_FOR_CHILD_JOB_ONLY);
				}

				parentContextMap_tRunJob_2.put("recipientEmail", obj_tRunJob_2);
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_12", "", Thread.currentThread().getId() + "", "ERROR",
							"", "Validation KO", "", "");
					talendLogs_LOGS.addMessage("tWarn", "tWarn_12", 5, "Validation KO", 4);
					talendLogs_LOGSProcess(globalMap);
					globalMap.put("tWarn_12_WARN_MESSAGES", "Validation KO");
	...
	public void tDie_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					tLogCatcher_3.addMessage("tDie", "tDie_1", 5, "Job en erreur", 4);
					tLogCatcher_3Process(globalMap);

					talendLogs_LOGS.addMessage("tDie", "tDie_1", 5, "Job en erreur", 4);
					talendLogs_LOGSProcess(globalMap);

					globalMap.put("tDie_1_DIE_PRIORITY", 5);
					System.err.println("Job en erreur");

					globalMap.put("tDie_1_DIE_MESSAGE", "Job en erreur");
					globalMap.put("tDie_1_DIE_MESSAGES", "Job en erreur");
	...
	public void tRunJob_6Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				obj_tRunJob_6 = ((String) globalMap.get("tFileList_1_CURRENT_FILE"));
				if (obj_tRunJob_6 != null) {
					paraList_tRunJob_6
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					resumeUtil.addLog("USER_DEF_LOG", "NODE:tWarn_5", "", Thread.currentThread().getId() + "", "INFO",
							"", "Send mail on error OK", "", "");
					talendLogs_LOGS.addMessage("tWarn", "tWarn_5", 3, "Send mail on error OK", 3);
					talendLogs_LOGSProcess(globalMap);
					globalMap.put("tWarn_5_WARN_MESSAGES", "Send mail on error OK");
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_connectionStatsLogs = routines.system.PasswordEncryptUtil
						.decryptPassword(
								"ENC:[S5etiGJj4iYwRdPk5hD3uUEgVAkxsSBDK69yRMWYJDu7ztqy3l/4Oy6upyRCEqtpK/b9B+LkK8A=]");
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = MASTERFILE_INTEGRATION_BDD.class.getClassLoader()
					.getResourceAsStream("dev/masterfile_integration_bdd_0_33/contexts/" + contextStr + ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 288781 characters generated by Talend Open Studio for Data Integration on the
 * 2 novembre 2022 11:29:09 CET
 ************************************************************************************************/